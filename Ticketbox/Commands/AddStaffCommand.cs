...
{
    internal class AddStaffCommand : ApplicationCommandModule
    {
        #region [Add Staff]

        [SlashCommand("addstaff", "Adds a new staff member.")]
        public static async Task AddStaff(InteractionContext ctx,
            [Option("user", "User to add to staff.")] DiscordUser user)
        {
            ...
            if (!await Database.AddStaff(new Staff { MemberId = member.Id, Name = member.DisplayName }))
            {
                await ctx.EditResponseAsync(new DiscordWebhookBuilder().AddEmbed(new DiscordEmbedBuilder()
                {
                    Title = "An error occurred.",
                    Color = DiscordColor.Red,
                    Description = "The staff was not added to the database! Please try again or contact the developer."
                }));
            }
            ...
            await ctx.EditResponseAsync(new DiscordWebhookBuilder().AddEmbed(new DiscordEmbedBuilder()
            {
                ...
                Description = $"Staff {member.Mention} has been successfully added!"
            }));
        }
